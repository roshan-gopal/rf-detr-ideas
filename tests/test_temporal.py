...
from rfdetr.graph import EDGE_DIM, GraphFrame, NODE_DIM, PickAndRollGNN
...
def test_temporal_encoder_output_shape() -> None:
    """Encoder maps (B, T, D) to (B, D)."""
    enc = PickAndRollTemporalEncoder(embed_dim=32, num_heads=4, num_layers=1, dim_feedforward=64)
...
def test_temporal_encoder_padding_mask_runs() -> None:
    ...
    enc = PickAndRollTemporalEncoder(embed_dim=32, num_heads=4, num_layers=2, dim_feedforward=64)
...
def test_temporal_classifier_logit_shape() -> None:
    """Classifier outputs one logit per batch item."""
    clf = PickAndRollTemporalClassifier(embed_dim=32, num_heads=4, num_layers=1, dim_feedforward=64)
...
def test_encode_graph_sequence_shape() -> None:
    """encode_graph_sequence returns (1, T, D) and matching padding mask."""
    ...
    x, pad = encode_graph_sequence(gnn, graphs)
...
def test_encode_graph_sequence_marks_invalid_frames() -> None:
    ...
    _x, pad = encode_graph_sequence(gnn, graphs)
