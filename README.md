...
## Pick-and-roll feature pipeline (this fork)
...
- **`scripts/filter_tactical_jsonl.py`** — filter exported JSONL to ball + player classes, optional NMS, remap possession IDs.
- **`scripts/run_tactical_graph_from_jsonl.py`** — replay filtered JSONL through **`PlayerTracker`** and **`PickAndRollGraphBuilder`**, optionally **`--save-graphs`** to write **`graph_features`** (and node/edge breakdown) for training or inspection, and **`--show-features`** to print vectors.

Training the temporal classifier **requires labeled clips** (or another supervisory signal you define). The graph builder’s roles are **priors** for structuring features, not ground-truth tactical labels.
